        ...
        */
        if (!r_cb.empty()) {
            ...
            for (auto & is : r_cb) {
                ...
                */
                glUniform3fv(phong_shader["uDiffuseColor"], 1, &is.mtl.diffuse[0]);
                glUniform3fv(phong_shader["uSpecularColor"], 1, &is.mtl.specular[0]);
                glUniform3fv(phong_shader["uAmbientColor"], 1, &is.mtl.ambient[0]);
                glUniform3fv(phong_shader["uEmissiveColor"], 1, &is.mtl.emission[0]);
                glUniform1f(phong_shader["uRefractionIndex"], is.mtl.ior);
                glUniform1f(phong_shader["uShininess"], is.mtl.shininess);
