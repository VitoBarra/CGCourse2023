...
bool test_sample(vec3 samp){
    float z = texture2D(uDepthMap, samp.xy).x;
    return z > samp.z;
}
...
void main(void)
{
    ...
    if (center.z>0.99){
        ...
        {
            samp = center + vec3(uSamples[i].x*uRadius/uSize.x, uSamples[i].y*uRadius/uSize.x, uSamples[i].z*uDepthScale);
            if (test_sample(samp))
